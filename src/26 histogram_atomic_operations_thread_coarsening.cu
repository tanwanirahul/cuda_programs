...
__global__ void histogram_kernel(unsigned char * image, unsigned int * bins, unsigned int width, unsigned int height) {
    ...
    unsigned int seg_start = (blockIdx.x * blockDim.x * COARSE_FACTOR);
    

    //Initialize a shared memory to hold the local output and initialize it to 0.
    __shared__ unsigned int private_bins[NUM_BINS];
    if(threadIdx.x < NUM_BINS) {
        private_bins[threadIdx.x] = 0.0;
    }
    __syncthreads();

    for(unsigned int c=0; c<COARSE_FACTOR; c++) {
        unsigned int i = seg_start + (BLOCK_SIZE * c) + threadIdx.x;
        if (i < width * height) {
    ...
    if(threadIdx.x < NUM_BINS && private_bins[threadIdx.x] > 0.0) {
...
void histogram_wrapper(unsigned char * image, unsigned int* bins, unsigned int width, unsigned int height) {
    ...
    unsigned int numBlocks = ((width * height) + (threadsPerBlock * COARSE_FACTOR) - 1) / (threadsPerBlock*COARSE_FACTOR); 
