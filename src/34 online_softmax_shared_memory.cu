...
__global__ void softmax_kernel(float *mat, float *result, unsigned int M, unsigned int N) {
    ...
    if(threadIdx.x == 0) {
        ...
        rowSum_s = 0.0;
        rowMax_s = -INFINITY;
        float rowMax = rowMax_s;
        float rowSum = rowSum_s;

        // Compute the rowMax and rowSum in an online fashion.
        for(unsigned int i=0; i<N; i++)
        {
            float current = rowData[i];
            float newMax = ( current > rowMax)? current: rowMax;
            rowSum = rowSum * exp(rowMax - newMax) + exp(current - newMax);
            rowMax = newMax;
        }
    ...
    for(unsigned int i=0; i<(N + blockDim.x -1)/blockDim.x; i++) {
        ...
        if(elementToWrite < N) {
            result[rowStart + elementToWrite] = exp(rowData[elementToWrite] - rowMax_s) / rowSum_s;
        }
