 ...
 * 
 * This implementation adds upon the optimized implementation using shared memory by implementing thread coarsening.
 * Shared memory is limited in size and may become contention if there are too many
 * threads launched in parallel. 
 */
__global__ void reduce_kernel(float *input, float *block_wise_sums, unsigned int N) {

    unsigned int seg_start = (blockIdx.x * blockDim.x * 2 * COARSE_SIZE);
    unsigned int seg_thread = seg_start + (threadIdx.x);

    __shared__ float input_s[BLOCK_SIZE];

    if (seg_thread + BLOCK_SIZE < N - (2 * COARSE_SIZE)) {
        float sum = 0.0;
        for(int i =0; i<2*COARSE_SIZE; i++) {
            sum+=input[seg_thread + (i*BLOCK_SIZE)];
        }
        ...
        if(threadIdx.x == 0) {
            block_wise_sums[blockIdx.x] = input_s[0];
        }
    }
}
...
void reduce_wrapper(float *input, float *result, unsigned int N)
{
    ...
    unsigned int numBlocks = (N + (2 * threadsPerBlock * COARSE_SIZE) - 1) / (threadsPerBlock * 2 * COARSE_SIZE);
