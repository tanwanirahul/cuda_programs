 ...
 * 
 * The Naive implementation suffers from two problems:
 * 1. Access to global memory aren't coalesced since each thread instead of accessing
 * sequential memory, accessess an alternate element at every time step.
 * 2. Control Divergence - At every step, no. of active threads keep reducing within a warp,
 * however since there are other active threads, the warp cannot be released. It would be ideal
 * to have a mechanism where the threads that are done computing belong to the same warp so
 * that entire warp could finish and be released from resources.
 */
__global__ void reduce_kernel(float *input, float *block_wise_sums, unsigned int N) {
    ...
    unsigned int seg_thread = seg_start + (threadIdx.x);

    if (seg_thread < N - BLOCK_SIZE) {
        for (int step = BLOCK_SIZE; step >= 1; step/=2 ) {
            if( threadIdx.x < step) {
...
int main(int argc, char **argv)
{
    ...
    unsigned int N = (argc > 1)?(atoi(argv[1])):(1 << 31);
