//%%writefile GEMM_using_mma_tiling.cu
/**
 * Genralized Matrix Multiplication (GEMM) using the CuTe Tensors
 ...
 use the Atom that exposes one of mma instructions supported by sm75 device.
  ...
  In addition, we will tile these MMA instructions and launch more than 32 threads per block 
...
// Define the WARP threds range. We will use this to partition the A and B input matrices.
#define WARP0 (threadIdx.x >= 0 && threadIdx.x < 32)
#define WARP1 (threadIdx.x >= 32 && threadIdx.x < 64)
#define WARP2 (threadIdx.x >= 64 && threadIdx.x < 96)
#define WARP3 (threadIdx.x >= 96 && threadIdx.x < 128)
