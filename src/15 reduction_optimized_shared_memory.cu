 ...
 * 
 * This implementation adds upon the optimized implementation by utilizing shared memory
 * instead of accessing global memory.
 */
__global__ void reduce_kernel(float *input, float *block_wise_sums, unsigned int N) {
    ...
    if (seg_thread < N - BLOCK_SIZE) {
        ...
        input_s[threadIdx.x] = input[seg_thread] + input[seg_thread + BLOCK_SIZE];
        __syncthreads();
