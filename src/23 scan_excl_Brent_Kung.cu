...
__global__ void add_kernel(float * output, float * block_wise_sums, unsigned int N) {
    ...
    if(blockIdx.x > 0) {
        if (i < N)
            output[i] += block_wise_sums[blockIdx.x];
        if (i + BLOCK_SIZE < N)
            output[i + BLOCK_SIZE] += block_wise_sums[blockIdx.x];
...
void scan_wrapper_d(float *input_d, float * output_d, unsigned int N) {
    ...
    cudaMemcpy(blockWiseSums_CPU, blockWiseSums_d, sizeof(float) * numBlocks, cudaMemcpyDeviceToHost);
