...
__global__ void stencil_kernel(float * input, float * output, unsigned int N) {
    ...
    int i_start = blockIdx.z * OUTPUT_TILE_SIZE;
    int j = blockIdx.y * OUTPUT_TILE_SIZE + threadIdx.y - 1;
    int k = blockIdx.x * OUTPUT_TILE_SIZE + threadIdx.x - 1;
    
    __shared__ float input_tile_prev[INPUT_TILE_SIZE][INPUT_TILE_SIZE];
    __shared__ float input_tile_curr[INPUT_TILE_SIZE][INPUT_TILE_SIZE];
    __shared__ float input_tile_next[INPUT_TILE_SIZE][INPUT_TILE_SIZE];

    // load input tiles.
    if(i_start >= 1 && i_start < N + 1 && j >= 0 && j < N && k >= 0 && k < N) {
        input_tile_prev[threadIdx.y][threadIdx.x] = input[(((i_start-1)*N+j)*N+k)];
    }

    if(i_start >= 0 && i_start < N && j >= 0 && j < N && k >= 0 && k < N) {
        input_tile_curr[threadIdx.y][threadIdx.x] = input[(((i_start)*N+j)*N+k)];
    }

    //__syncthreads();
    for(int i=i_start; i<i_start+OUTPUT_TILE_SIZE; i++)
    {
        if( (i+1) >= 0 && (i+1) < (N) && j >= 0 && j < N && k >= 0 && k < N) {
            input_tile_next[threadIdx.y][threadIdx.x] = input[(((i+1)*N+j)*N+k)];
        }
        __syncthreads();

         // compute stencil.
        if(i>=1 && i<(N-1) && j>=1 && j<(N-1) && k>=1 && k<(N-1) ) {
            if(threadIdx.x >= 1 && threadIdx.x < INPUT_TILE_SIZE-1 && threadIdx.y >= 1 && threadIdx.y < INPUT_TILE_SIZE-1) {
                output[(i*N+j)*N+k] = C0 * (input_tile_curr[threadIdx.y][threadIdx.x]) +
                    C1 * ((input_tile_curr[threadIdx.y][threadIdx.x - 1]) +
                        (input_tile_curr[threadIdx.y][threadIdx.x +1]) +
                        (input_tile_curr[threadIdx.y - 1][threadIdx.x]) +
                        (input_tile_curr[threadIdx.y + 1][threadIdx.x]) +
                        (input_tile_prev[threadIdx.y][threadIdx.x]) +
                        (input_tile_next[threadIdx.y][threadIdx.x]));
            }
        }
        __syncthreads();
        input_tile_prev[threadIdx.y][threadIdx.x] = input_tile_curr[threadIdx.y][threadIdx.x];
        input_tile_curr[threadIdx.y][threadIdx.x] = input_tile_next[threadIdx.y][threadIdx.x];
    }
}
    ...
    for(int i= 1; i<N-1; i++) {
        for(int j= 1; j<N-1; j++) {
            for(int k= 1; k<N-1; k++) {
                ...
                {
                    areEqual = 0;
                    printf("Mismatch found at: i:%d, j:%d, k:%d\n",i,j,k);
                }
            }
        }   
    }
